...
export async function createCatalogAlbum(input: { title: string; coverImage: string; vinylImage?: string; releaseYear?: number; trackFormat?: string; sortOrder?: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
export async function updateCatalogAlbum(input: { id: number; title: string; coverImage: string; vinylImage?: string; releaseYear?: number; trackFormat?: string; sortOrder?: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  await db.update(catalogAlbums).set({ title: input.title, coverImage: input.coverImage, vinylImage: input.vinylImage, releaseYear: input.releaseYear, trackFormat: input.trackFormat, sortOrder: input.sortOrder ?? 0 }).where(eq(catalogAlbums.id, input.id));
}
