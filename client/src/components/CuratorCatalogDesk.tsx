...
type Album = { id: number; title: string; coverImage: string; vinylImage?: string | null; releaseYear?: number | null; trackFormat?: string | null; tracks: Track[] };
...
export default function CuratorCatalogDesk({ catalog }: { catalog: Album[] }) {
  ...
  const [albumTrackFormat, setAlbumTrackFormat] = useState("");
  ...
  const addAlbum = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createAlbum.mutateAsync({ title: albumTitle, coverImage, vinylImage: vinylImage || undefined, releaseYear: albumReleaseYearFromInput(albumReleaseYear), trackFormat: albumTrackFormat || undefined });
      setAlbumTitle(""); setCoverImage(""); setVinylImage(""); setAlbumReleaseYear(""); setAlbumTrackFormat("");
  ...
  const saveAlbum = async (event: FormEvent<HTMLFormElement>, album: Album) => {
    ...
    try {
      await updateAlbum.mutateAsync({ id: album.id, title: String(data.get("title")), coverImage: String(data.get("coverImage")), vinylImage: String(data.get("vinylImage") || "") || undefined, releaseYear: albumReleaseYearFromInput(data.get("releaseYear")), trackFormat: String(data.get("trackFormat") || "") || undefined });
        ...
        return <article className={className} key={album.id} onDragOver={event => { if (!isFiltered) event.preventDefault(); }} onDrop={() => { if (!isFiltered) dropAlbum(album.id); }}>
          ...
          {isExpanded && <div id={`sleeve-editor-${album.id}`} className="desk-sleeve-editor">
            <form className="desk-album" onSubmit={event => void saveAlbum(event, album)}>
              ...
              <label>Album title<input name="title" defaultValue={album.title} required /></label><label>Release year<input name="releaseYear" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={4} defaultValue={album.releaseYear ?? ""} placeholder="e.g. 2002" /></label><label>Track format<input name="trackFormat" defaultValue={album.trackFormat ?? ""} placeholder="e.g. Movie Soundtrack, EP, Single" /></label><label>DVD / sleeve image URL<input name="coverImage" defaultValue={album.coverImage} required /></label><label>Full vinyl image URL<input name="vinylImage" defaultValue={album.vinylImage ?? ""} /></label><button type="submit" disabled={updateAlbum.isPending}><Save size={14} /> Update artwork</button></form>
