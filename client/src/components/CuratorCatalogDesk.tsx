...
type Album = { id: number; title: string; subtitle?: string | null; coverImage: string; vinylImage?: string | null; releaseYear?: number | null; tracks: Track[] };
...
export default function CuratorCatalogDesk({ catalog }: { catalog: Album[] }) {
  ...
  const addAlbum = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createAlbum.mutateAsync({ title: albumTitle, subtitle: albumSubtitle || undefined, coverImage, vinylImage: vinylImage || undefined, releaseYear: albumReleaseYearFromInput(albumReleaseYear) });
      setAlbumTitle(""); setAlbumSubtitle(""); setCoverImage(""); setVinylImage(""); setAlbumReleaseYear("");
  ...
  const saveAlbum = async (event: FormEvent<HTMLFormElement>, album: Album) => {
    ...
    try {
      await updateAlbum.mutateAsync({ id: album.id, title: String(data.get("title")), subtitle: String(data.get("subtitle") || "") || undefined, coverImage: String(data.get("coverImage")), vinylImage: String(data.get("vinylImage") || "") || undefined, releaseYear: albumReleaseYearFromInput(data.get("releaseYear")) });
